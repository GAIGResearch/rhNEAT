...
public class ArcadeMachine {
	...
	public static double[] runOneGame(String game_file, String level_file, boolean visuals, String agentNames,
		...
		double[] score;
		if (visuals)
			score = toPlay.playGame(null, players, randomSeed, anyHuman, playerID);
		else
	...
	public static double[] runOneExpGame(JFrame frame, String game_file, String level_file, boolean visuals,
										 String agentNames, String actionFile, String evoFile, int randomSeed, int playerID, ParameterSet params)
	{
		...
		for (int i = 0; i < no_players; i++) {
			...
			if (no_players > 1) {
				//multi player
				players[i] = ArcadeMachine.createMultiPlayer(names[i], actionFile, toPlay.getObservationMulti(i), randomSeed, i, humans[i]);
			} else {
				//single player
		...
		double[] score;
		if(visuals)
			score = toPlay.playGame(frame, players, randomSeed, anyHuman, playerID);
		else
			score = toPlay.runGame(players, randomSeed);
	...
	public static double runOneGeneratedLevel(String gameFile, boolean visuals, String agentName, String actionFile,
		...
		if (visuals)
			score = toPlay.playGame(null, p, randomSeed, isHuman, 0)[0];
		else
	...
	public static double[] replayGame(String game_file, String level_file, boolean visuals, String actionFile) {
		...
		double[] score;
		if (visuals)
			score = toPlay.playGame(null, players, seed, false, 0);
		else
	...
	public static boolean tearPlayerDown(Game toPlay, Player[] players, String actionFile, int randomSeed,
		...
		for (Player p : players) {
			...
			// Inform about the result and the final game state.
			if (toPlay.no_players > 1)
				p.resultMulti(toPlay.getObservationMulti(0).copy(), ect);
			else
